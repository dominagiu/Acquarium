...
def calculate_metrics(actual, predicted):
    """
    Calcola le metriche di performance del modello, tra cui:
    - True Positive Rate (TPR)
    - False Discovery Rate (FDR)
    - Fraction of Time Alerting (FTA)
    - Average Lead Time (tempo medio di preavviso prima dell'evento)

    Args:
        actual (list): Lista di tuple con intervalli reali [(start_time, end_time), ...]
        predicted (list): Lista di tuple con intervalli predetti [(start_time, end_time), ...]

    Returns:
        tuple: (TPR, FDR, FTA, avg_lead_time)
    """
    ...
    # Convertiamo le date in formato datetime per evitare problemi di confronto
    ...
    for pred_start, pred_end in predicted_intervals:
        ...
        overlapping = False  # Indica se l'evento predetto si sovrappone a un'eruzione reale
        too_early = True  # Controllo se l'evento è più di 6 ore prima dell'eruzione reale

        for act_start, act_end in actual_intervals:
            if (pred_start <= act_end) and (pred_end >= act_start):  # Controllo sovrapposizione
                overlapping = True
                true_positives += 1

                # **Calcolo del lead time corretto**
                lead_time = (act_start - pred_start).total_seconds() / 3600  # In ore
                
                if lead_time <= 6:  # ✅ Escludiamo predizioni troppo anticipate
                    lead_times.append(lead_time)

                # **Calcoliamo il tempo di allerta corretto**
                correct_alert_time += (min(pred_end, act_end) - max(pred_start, act_start)).total_seconds()
                too_early = False  # Se si sovrappone, non è un falso positivo anticipato
                break  # Se abbiamo trovato una sovrapposizione, non serve controllare altre eruzioni

            # **Se pred_start è più di 6 ore prima di un evento reale, segna come troppo anticipato**
            if (act_start - pred_end).total_seconds() / 3600 < 6:
                too_early = False  

        # **Se non si sovrappone a nessun evento reale, è un falso positivo**
        if not overlapping:
            false_positives += 1
        elif too_early:  # **Se è predetto con oltre 6 ore di anticipo e non si sovrappone, è un falso positivo**
            false_positives += 1

    # **Calcoliamo i falsi negativi (eruzioni reali non predette)**
    for act_start, act_end in actual_intervals:
    ...
    print(f"📊 Risultati metriche: TPR: {TPR:.4f}, FDR: {FDR:.4f}, FTA: {FTA:.4f}, Average Lead Time: {avg_lead_time:.2f} ore")

    # **Salviamo le metriche in un file CSV per analisi future**
    metrics_file = 'model_metrics_LSTM_final.csv'
...
def save_predicted_events(y_pred_proba, timestamps, threshold=0.8, output_csv="2D_predicted_events.csv"):
    """
    ...
    Args:
        ...
        threshold (float): Soglia per considerare un evento predetto (default=0.8).
...
def tune_threshold(y_pred_proba, timestamps, actual_intervals, thresholds=np.arange(0.1, 0.85, 0.05), output_csv="2DCNN_tuning_results.csv"):
    """
    Testa diversi valori di threshold per valutare l'impatto sulle metriche.

    Args:
        y_pred_proba (array): Probabilità predette dal modello.
        timestamps (array): Timestamp associati alle predizioni.
        actual_intervals (list): Lista di tuple con intervalli reali [(start_time, end_time), ...].
        thresholds (array): Valori di threshold da testare.
        output_csv (str): Nome del file CSV in cui salvare i risultati.
    """
    ...
    for threshold in thresholds:
        ...
        # Convertiamo le probabilità in valori binari
        y_pred_binary = (y_pred_proba > threshold).astype(int)
        ...
        # Calcoliamo le metriche usando la funzione già definita
...
model_save_path = 'cnn_polling_model.h5'
...
save_predicted_events(y_pred_proba, timestamps, threshold=0.8, output_csv="2DCNN_predicted_events.csv")
