...
def build_lstm_model():
    input_infra = Input(shape=(n_timesteps, n_features_infra))
    x_infra = LSTM(64, return_sequences=True, dropout=0.3)(input_infra)
    x_infra = LSTM(32, return_sequences=False, dropout=0.3)(x_infra)

    input_seismic = Input(shape=(n_timesteps, n_features_seismic))
    x_seismic = LSTM(64, return_sequences=True, dropout=0.3)(input_seismic)
    x_seismic = LSTM(32, return_sequences=False, dropout=0.3)(x_seismic)
...
def save_predicted_events(y_pred_proba, timestamps, threshold=0.5, output_csv="LSTM_predicted_events.csv"):
    """
...
def tune_threshold(y_pred_proba, timestamps, actual_intervals, thresholds=np.arange(0.1, 0.85, 0.05), output_csv="LSTM_tuning_results.csv"):
    """
...
model_save_path = 'lstm_model_final.h5'
if os.path.exists(model_save_path):
    ...
    model.fit(X_train, y_train, epochs=10, batch_size=32, validation_split=0.2, class_weight=class_weight_dict, callbacks=[checkpoint])
...
save_predicted_events(y_pred_proba, timestamps, threshold=0.1, output_csv="LSTM_predicted_events.csv")
